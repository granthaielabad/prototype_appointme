...
class AuthController extends Controller
{
    ...
    public function login(): void
    {
        ...
        if ((int)$user['is_active'] !== 1) {
            Session::flash('error', 'Account not activated. Please verify your email with the OTP sent during registration.', 'danger');
        ...
        header('Location: /my-appointments'); exit;
    }
    ...
    public function register(): void
    {
        ...
        Session::flash('success', 'Account created. A verification code has been sent to your email. Please verify to activate your account.', 'success');
        header('Location: /verify-otp?email=' . urlencode($email)); exit;
    }
    ...
    public function verifyOtp(): void
    {
        ...
        if ($email === '' || $code === '') {
            Session::flash('error','Please enter email and code','danger');
            header('Location: /verify-otp?email='.urlencode($email)); exit;
        }

        $otpModel = new OTP();
        $row = $otpModel->getValidOtp($code, null, $email);
        if (!$row) {
        ...
        if ($user) {
            ...
            Session::flash('success','Account verified. You can now login.','success');
            header('Location: /login'); exit;
        }
    ...
    public function sendResetLink(): void
    {
        ...
        $resetLink = sprintf('%s/reset-password?token=%s', $_ENV['APP_URL'] ?? 'http://localhost/prototype', $resetToken);
    ...
    public function resetPassword(): void
    {
        ...
        if ($password === '' || $confirm === '') {
            ...
            header('Location: /reset-password?token=' . urlencode($resetToken)); exit;
        }
